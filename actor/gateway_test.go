...
func TestFetcherAndRemover(t *testing.T) {
	...
	fetcherProp := NewFetcherGroup(queue, queueURL, 1)
	fetcher := sys.Root.Spawn(fetcherProp)
	...
	removerProp := NewRemoverGroup(queue, queueURL, 33)
	remover := sys.Root.Spawn(removerProp)
